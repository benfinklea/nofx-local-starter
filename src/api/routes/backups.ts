...
export default function mount(app: Express){
  app.get('/backups', async (req, res): Promise<void> => {
    if (!isAdmin(req)) { res.status(401).json({ error: 'auth required', login: '/ui/login' }); return; }
    const rows = await listBackups();
    res.json(rows);
  });
  app.post('/backups', async (req, res): Promise<void> => {
    if (!isAdmin(req)) { res.status(401).json({ error: 'auth required', login: '/ui/login' }); return; }
  ...
  app.post('/backups/:id/restore', async (req, res): Promise<void> => {
    if (!isAdmin(req)) { res.status(401).json({ error: 'auth required', login: '/ui/login' }); return; }

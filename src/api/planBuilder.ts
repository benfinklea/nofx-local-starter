...
export async function buildPlanFromPrompt(
  ...
  const steps: StepDef[] = [];
  const gateSteps: StepDef[] = [];
  if (opts.quality) {
    if (gates.typecheck) gateSteps.push({ name: 'typecheck', tool: 'gate:typecheck' });
    if (gates.lint) gateSteps.push({ name: 'lint', tool: 'gate:lint' });
    if (gates.unit) gateSteps.push({ name: 'unit', tool: 'gate:unit' });
    const gx = gates as Record<string, unknown>;
    if (gx.sast === true) gateSteps.push({ name: 'sast', tool: 'gate:sast' });
    if (gx.secrets === true) gateSteps.push({ name: 'secret scan', tool: 'gate:secrets' });
    if (gx.audit === true) gateSteps.push({ name: 'dependency audit', tool: 'gate:audit' });
  }
  ...
  // Run gates after content has been generated (and optionally written into the workspace)
  if (gateSteps.length) {
    steps.push(...gateSteps);
  }

...
export const store = {
  ...
  deleteArtifactsByRun: (runId: string) => StoreFactory.getInstance().deleteArtifactsByRun?.(runId) ?? Promise.resolve(),
  deleteArtifactsByStep: (runId: string, stepId: string) => StoreFactory.getInstance().deleteArtifactsByStep?.(runId, stepId) ?? Promise.resolve(),

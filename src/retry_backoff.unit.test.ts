...
describe('Memory queue retry backoff and exactly-once (delayed)', () => {
  ...
  it('retries with backoff [0,2s,5s] and succeeds once on third attempt', async () => {
    ...
    subscribe(topic, async (payload: any) => {
    ...
    let c1 = await getCounts(topic);
    expect(attempts).toEqual([1]);
    // After failure, next job should be delayed (>=1 delayed)
    expect((c1 as any).delayed).toBeGreaterThanOrEqual(1);

    // Advance to run second attempt (2s)
    await vi.advanceTimersByTimeAsync(2001);
    let c2 = await getCounts(topic);
    expect(attempts).toEqual([1,2]);
    expect((c2 as any).delayed).toBeGreaterThanOrEqual(1);

    // Advance to run third attempt (5s)
    await vi.advanceTimersByTimeAsync(5001);
    const c3 = await getCounts(topic);
    expect(attempts).toEqual([1,2,3]);
    // Queue should be drained
    expect((c3 as any).waiting).toBe(0);
    expect((c3 as any).active).toBe(0);
